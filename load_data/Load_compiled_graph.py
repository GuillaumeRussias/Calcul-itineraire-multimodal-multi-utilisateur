...
def load_graph(scheduled_edges = PandaE ,free_edges = transferts , size = 0) :
    Graph = fg.graph(size)
    One = np.ones(free_edges['from_stop_id'].size)
    Graph.build_scheduled_edges(scheduled_edges['departure_stop_index'].to_numpy(),scheduled_edges['arrival_stop_index'].to_numpy(),scheduled_edges["departure_time"].to_numpy(),scheduled_edges["arrival_time"].to_numpy(),scheduled_edges.index.to_numpy())
    Graph.build_free_edges(free_edges['from_stop_id'].to_numpy(),free_edges['to_stop_id'].to_numpy(),free_edges["min_transfer_time"].to_numpy(),-One)
    return Graph
